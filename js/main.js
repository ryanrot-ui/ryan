...
function applyLang(lang) {
  ...
  // Review cards — querySelectorAll is reliable here; nth-child misfires on this structure
  const reviewData = [
    {
      ratings: ['Food 5/5', 'Service 5/5', 'Ambiance 4/5'],
      ratingsJa: ['料理 5/5', 'サービス 5/5', '雰囲気 4/5'],
      text: '"More relaxed than other spots in Shin-Okubo, with top-tier food quality in the area. Staff are friendly, the space is clean, and the music goes beyond K-pop. Highly recommended for groups who want to take their time over drinks."',
      textJa: '「新大久保の他の飲み屋より落ち着いた雰囲気で、料理のクオリティは周辺でもトップクラス。スタッフも感じよく、店内も清潔。音楽もK-POPだけじゃなくて、グループでゆっくり飲みたい人に本当におすすめです。」',
      name: 'Kobayashi Hana', nameJa: '小林 花',
      sub: 'Google Review · Local Guide', subJa: 'Googleレビュー · 地元のガイド',
    },
    {
      ratings: ['Food 5/5', 'Service 5/5', 'Ambiance 5/5'],
      ratingsJa: ['料理 5/5', 'サービス 5/5', '雰囲気 5/5'],
      text: '"QR code ordering was convenient. The staff were incredibly attentive — they brought forks before we even asked, and comped us tiramisu. MIMI was especially wonderful. Delicious food at fair prices."',
      textJa: '「QRコードで注文できて便利。スタッフの方がとても親切で、フォークを頼む前に持ってきてくれたり、ティラミスのビンスをサービスしてくれたり。MIMIさんの対応が特に素晴らしかったです。料理も美味しくて価格も納得。」',
      name: 'Sato Mei', nameJa: '佐藤 めい',
      sub: 'Google Review · Visited', subJa: 'Googleレビュー · 来店済み',
    },
    {
      ratings: ['Group of 3–4', 'All-you-can-drink course'],
      ratingsJa: ['3〜4名で来店', '飲み放題コース利用'],
      text: '"Ordered the samgyeopsal set — great value at ¥3,000, and only ¥4,299 with all-you-can-drink. The straw-smoked pork had a unique smoky depth unlike anywhere else. The pajeon was crispy with big shrimp, and the jjigae was rich and hearty."',
      textJa: '「サムギョプサルセットを注文。セットだけで¥3,000、飲み放題込みで¥4,299と大満足。藁焼きのサムギョプサルは他と全然違う燻煙の香りで美味しかった。チヂミも大きなエビが入っていて外はカリッと。チゲもご飯が進む濃厚な味。」',
      name: 'Yamamoto Shinichi', nameJa: '山本 慎一',
      sub: 'Google Review · All-you-can-drink course', subJa: 'Googleレビュー · 飲み放題コース利用',
    },
  ];
  document.querySelectorAll('.testimonial-card').forEach((card, i) => {
    const d = reviewData[i];
    if (!d) return;
    const ratingSpans = card.querySelectorAll('.review-ratings span');
    ratingSpans.forEach((sp, j) => {
      sp.textContent = lang === 'en' ? (d.ratings[j] || sp.textContent) : (d.ratingsJa[j] || sp.textContent);
    });
    const p = card.querySelector('p');
    if (p) p.innerHTML = lang === 'en' ? d.text : d.textJa;
    const strong = card.querySelector('.testimonial-author strong');
    if (strong) strong.textContent = lang === 'en' ? d.name : d.nameJa;
    const span = card.querySelector('.testimonial-author span');
    if (span) span.textContent = lang === 'en' ? d.sub : d.subJa;
  });
