...
# Revealed in beam → MAX jumpscare → then slowly walks closer.
...
const LOOK_THRESHOLD = 0.86   # direct gaze dot product
...
var _revealed: bool  = false  # beam-reveal jumpscare fires only once
...
# Called by GhostSpawnDirector during forced flicker (scare 3)
func teleport_close_silent() -> void:
	_player = GameManager.player_ref
	if not _player:
		return
	# Place just outside easy-look range, slightly to the side
...
func _handle_stalk(delta: float) -> void:
	...
	# Classic mechanic: freeze when LOOKED AT in the beam
	...
	# Drift toward player while not watched
